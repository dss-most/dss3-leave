...
%>
<div id="doc">
<h3 class="center">แบบใบลาอุปสมบท</h3>
...
เกิดวันที่ <%=date.getDate(employee.birthDate, date.MONTH_NAME_FULL).get(date.DATE_THAI).toString()%>
เข้ารับราชการเมื่อวันที่ <%=date.getDate(employee.startWorkDate, date.MONTH_NAME_FULL).get(date.DATE_THAI).toString()%>
</p>
<p class="docBody">
ข้าพเจ้า <%=thisForm.getIsLeaveBeforeName(thisForm.getValue(thisForm.ELM_NAME_IS_LEAVE_BEFORE))%>อุปสมบท
บัดนี้มีศรัทธาจะอุปสมบทในพระพุทธศาสนา
ณ วัด <%=thisForm.getValue(thisForm.ELM_NAME_TEXT_1)%>
ตั้งอยู่ ณ <%=thisForm.getValue(thisForm.ELM_NAME_TEXT_2)%>
กำหนดวันที่ <%=date.getDate(thisForm.getValue(thisForm.ELM_NAME_DETAIL_DATE), date.MONTH_NAME_SHORT).get(date.DATE_THAI).toString()%>
และจะจำพรรษาอยู่ ณ วัด <%=thisForm.getValue(thisForm.ELM_NAME_TEXT_3)%>
ตั้งอยู่ ณ <%=thisForm.getValue(thisForm.ELM_NAME_TEXT_4)%>
จึงขออนุญาตลาอุปสมบทมีกำหนด <%=thisForm.getValue(thisForm.ELM_NAME_LEAVE_DAY_COUNT_MASTER)%> วัน
